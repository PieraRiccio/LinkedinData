...
merge (data.matrix, data.matrix2, by="m_urn", sort=TRUE)
```
...
names <- as.vector(dplyr::select(data.matrix, m_urn))

pca.data <- data.frame(names,
  X=pca$x[,1],
  Y=pca$x[,2])
pca.data
...
label <- dplyr::select(pca.data, m_urn)
head((names))
dim((names))

ggplot(data=pca.data, aes(x=X, y=Y, label=names)) +
  geom_text() +
...
top_10_genes <- names(gene_score_ranked[1:7])
