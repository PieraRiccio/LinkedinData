...
data.matrix2 <- select (data.matrix, -c(c_name, gender, ethnicity, glass, img, nationality, m_urn, X))
data.matrix2
...
```{r}
pca <- prcomp(t(data.matrix2), scale=TRUE)
## plot pc1 and pc2
...
#names <- seq(1, 49, by=1)
features <- colnames(data.matrix2)

pca.data <- data.frame(features,
  X=pca$x[,1],
  Y=pca$x[,2])
pca.data

X <- dplyr::select(pca.data, X)
Y <- dplyr::select(pca.data, Y)
label <- dplyr::select(pca.data, features)
head((names))
dim((names))

ggplot(data=pca.data, aes(x=X, y=Y, label=features)) +
  geom_text() +
...
feature_scores <- abs(loading_scores) ## get the magnitudes

feature_score_ranked <- sort(feature_scores, decreasing=TRUE)
top_10_feature <- names(feature_score_ranked[1:2])
 
top_10_feature ## show the names of the top 10 genes
 
pca$rotation[top_10_feature,1] ## show the scores (and +/- sign)
...
svd.stuff <- svd(scale(data.matrix2, center=TRUE))
 
## calculate the PCs
svd.data <- data.frame(Sample=colnames(data.matrix2),
