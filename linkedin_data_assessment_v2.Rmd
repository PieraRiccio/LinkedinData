...
h_male_high <- ggplot(data=filter(df, gender =="Male"), aes(tenure_len)) + 
        geom_histogram(breaks=seq(3000, 22000, by=1000), 
                 col="violet", 
                 fill="pink", 
                 alpha = .9) + 
        labs(title="Uomini assunti", x="Durata dell'incarico (ore)", y="Numero assunti") + 
...
```{r}
```{r}
plot(df$smile, df$tenure_len, main = "Correlazione tra il sorriso mostrato in foto e la lunghezza dell'incarico ottenuto", col = "Red")
```
Si può evincere che non c'è correlazione tra la lunghezza dell'incarico e il sorriso
```
