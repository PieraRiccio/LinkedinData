...
In effetti, immaginando di visualizzare un dataset come un insieme di coordinate in un sistema a più dimensioni, attraverso l'utilizzo di solo alcune delle componenti principali ottenute mediante la PCA, è possibile ridurre la dimensionalità del dataset conservando tuttavia quante più informazioni possibili sulle osservazioni stesse. 

A seguito di queste premesse, si è in grado di pensare al processo della PCA come l’adattamento a un ellissoide dei dati presi in considerazione, dove ciascun asse rappresenta una componente principale. 
Poiché la varianza lungo un asse è direttamente proporzionale alla lunghezza di quest'ultimo, è possibile ometterlo (e di conseguenza anche la corrispondente componente principale) consentendo così di perdere solo un proporzionalmente piccolo quantitativo di informazioni.

I risultati della PCA spesso rivelano la struttura interna dei dati, facendo risaltare in modo evidente la varianza delle osservazioni. 


Nel caso specifico del dataset in esame, risulta difficile identificare quali, tra le decine di attributi che caratterizzano ciascun lavoratore, influenzi concretamente l’andamento del dataset. Di conseguenza, si adopera la tecnica di PCA, per determinare quali lavoratori risultino simili tra loro e quali, invece, risultino totalmente differenti; analizzando attributi ritenuti opportuni a tale scopo.  
...
Tale tracciato risulta essere fondamentale nella determinazione del numero di PC da utilizzare per un'analisi adeguata del dataset. 
...
Così come nell'analisi manuale, quindi, è possibile concludere che esiste nel dataset un leggera discriminazione di genere, che tuttavia, almeno attenendosi alle indagini qui svolte, sembra essere trascurabile per quanto concerne l'effettiva assunzione da parte delle aziende presentate.  
